<script lang="ts">
	...
	let data: ContactInfo | null = null;
	...
	onMount(async () => {
		try {
			...
			console.log(`Fetching data from: ${API_URL}/contact`);

			const contactReq = await axios.get(`${API_URL}/contact`);

			if (contactReq.status === 200) {
				data = contactReq.data;
			}
		} catch (error) {
...
<main class="landing-page px-4">
	...
	{:else}
		...
		<p class="text-2xl sm:text-3xl md:text-4xl mt-12">
			<span class="highlighted-text">{data?.title.split(" ")[0]}</span>{data?.title
				.split(" ")
				.slice(1)
				.join(" ")}
		</p>
		...
		<div class="flex flex-col gap-6 mt-12 items-center sm:flex-row sm:gap-12">
			<SocialIcon
				link={data?.email === undefined ? "" : `mailto:${data.email}`}
